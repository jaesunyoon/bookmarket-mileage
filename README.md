...
## 서비스 시나리오
1. Order 서비스에서 주문 시 isMile 값이 "Y" 일 경우 Mileage 서비스를 통해 결제가 진행되도록 한다.
1. Mileage 서비스를 통해 결재 진행 후 배송이 진행된다.
1. Order 서비스에서 isMile 값이 "Y" 인 주문의 취소 시 Mileage 결제가 취소된다.
1. Mileage 서비스를 통한 결제 취소 건에 대해서 배송이 취소된다.
1. Mileage를 사용하지 않은 결제 건이 배송완료될 경우 Mileage를 적립한다.
1. Mileage를 사용하지 않은 결제 건이 취소될 경우 Mileage를 적립을 취소한다.
...
## 헥사고날 아키텍처 변화
![image](https://user-images.githubusercontent.com/20619166/98310583-b1d4a200-2010-11eb-8b5b-2eab015c7181.png)


## DDD의 적용
- 도출된 Aggregate Root 객체를 Entity로 선언하였다.
```
package bookmarket;
...
siege -c1 -t30S -v --content-type "application/json" 'http://20.196.153.111:8080/orders POST {"bookId": "10", "qty": "1", "customerId": "1002", "isMile": "Y"}'
